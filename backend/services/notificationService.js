...
class NotificationService {
  static async getMyNotifications(userId, query) {
    ...
    const data = await NotificationModel.getMyNotifications(
      userId,
      limit,
      offset,
    );
  ...
  static async getSentNotifications(userId, query) {
    const page = Number(query.page) || 1;
    const limit = Number(query.limit) || 10;
    const offset = (page - 1) * limit;

    const data = await NotificationModel.getSentNotifications(
      userId,
      limit,
      offset,
    );
    const total = await NotificationModel.countSentNotifications(userId);
  ...
  static async create(body, user) {
    ...
    const notification = await NotificationModel.createNotification({
      ...
      loai:
        target.type === "student_ids"
          ? "ca_nhan"
          : target.type === "course_class"
            ? "lop"
            : "he_thong",
    });
    ...
    if (target.type === "course_class") {
      ...
      if (!user.vai_tro || !user.vai_tro.includes("admin")) {
        const [lop_mon] = await db.query(
          "SELECT giang_vien_id FROM lop_mon_hoc WHERE id = ?",
          [target.course_class_id],
        );
        ...
        if (gvRows.length === 0 || gvRows[0].id !== lop_mon[0].giang_vien_id) {
          throw new Error(
            "Bạn không có quyền gửi thông báo tới lớp này (Không phải giảng viên phụ trách)",
          );
        }
      }

      receiverIds = await NotificationModel.getUserIdsByCourseClass(
        target.course_class_id,
      );
    }
    ...
    if (target.type === "student_ids") {
      ...
      receiverIds = await NotificationModel.getUserIdsByStudentIds(
        target.student_ids,
      );
    }
  ...
  static async getDetail(notificationId, userId) {
    const notification = await NotificationModel.getDetail(
      notificationId,
      userId,
    );
