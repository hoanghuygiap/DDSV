...
class SessionModel {
    static buildFilterSql(filter = {}) {
        ...
        if (filter.keyword) {
            whereSql += `
                AND (
                    hp.ten_hoc_phan LIKE ?
                    OR lmh.ma_lop LIKE ?
                    OR ph.ten_phong LIKE ?
                )
            `;
        ...
        if (filter.date) {
            whereSql += ` AND bh.ngay_hoc = ? `;
            params.push(filter.date);
        }

        if (filter.week) {
            whereSql += `
                AND bh.ngay_hoc BETWEEN ? AND DATE_ADD(?, INTERVAL 6 DAY)
            `;
            params.push(filter.week, filter.week);
        }

        if (filter.status) {
            whereSql += ` AND bh.trang_thai = ? `;
            params.push(filter.status);
        }

        if (filter.room_id) {
            whereSql += ` AND bh.phong_hoc_id = ? `;
            params.push(filter.room_id);
        }

        if (filter.course_class_id) {
            whereSql += ` AND bh.lop_mon_hoc_id = ? `;
            params.push(filter.course_class_id);
        }

        return { whereSql, params };
    }

    static async getAll({ limit, offset, filter }) {
        const { whereSql, params } = this.buildFilterSql(filter);
