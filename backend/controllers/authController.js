  ...
  refreshToken: async (req, res) => {
    try {
      ...
      if (!refreshToken) {
        return res
          .status(400)
          .json({ success: false, message: "Yêu cầu cung cấp Refresh Token." });
      }
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        html: `
                ...
                <p>Token reset:</p>
                <p><b>${resetToken}</b></p>
                <p>Token có hiệu lực trong 10 phút.</p>
            `,
      });
    ...
    try {
      ...
      if (!resetToken || !newPassword) {
        return res
          ...
          .json({
            success: false,
            message: "Thiếu mã reset hoặc mật khẩu mới.",
          });
      }
      ...
      return res
        ...
        .json({
          success: false,
          message: "Mã xác thực reset không hợp lệ hoặc đã hết hạn.",
        });
    }
  },
  ...
  verifyToken: async (req, res) => {
    try {
      ...
      if (!token) {
        return res
          .status(400)
          .json({ success: false, message: "Yêu cầu token để xác thực." });
      }
      const decoded = jwt.verify(
        token,
        process.env.ACCESS_TOKEN_SECRET || "access_secret_key_123",
      ...
      return res
        .status(200)
        .json({ success: true, message: "Token hợp lệ.", data: decoded });
    } catch (error) {
      return res
        .status(401)
        .json({
