...
const fs = require("fs");
const { getPagination, pagingData } = require("../utils/pagination");
...
class CourseClassController {
  ...
  static async update(req, res) {
    try {
      const data = await CourseClassService.update(req.params.id, req.body);
      ...
      if (!studentId && ma_sinh_vien) {
        ...
        if (svRows.length === 0) {
          return res
            ...
            .json({
              success: false,
              message: "Không tìm thấy sinh viên có mã này",
            });
        }
  ...
  static async selfRegister(req, res) {
    try {
      await CourseClassService.selfRegister(req.params.id, req.user.id);
  ...
  static async selfUnregister(req, res) {
    try {
      await CourseClassService.selfUnregister(req.params.id, req.user.id);
  ...
  static async getMyClasses(req, res) {
    try {
      const data = await CourseClassService.getMyClasses(req.user.id);
