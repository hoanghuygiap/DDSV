...
function groupByMonth(rows: WeeklyRow[]) {
    ...
    rows.forEach(r => {
        ...
        if (!map[month]) {
            map[month] = { total: 0, co_mat: 0 }
        }

        map[month].total += Number(r.tong_buoi) || 0
        map[month].co_mat += Number(r.co_mat) || 0
    })
    ...
    return Object.entries(map)
        ...
        .map(([month, v]) => {
            const rate = v.total > 0
                ? Number(((v.co_mat / v.total) * 100).toFixed(1))
                : 0
...
export default function ReportsPage() {
    ...
    useEffect(() => {
        ...
        Promise.allSettled([
            ...
            .then(([classRes, warnRes, stuRes, weekRes]) => {
                ...
                if (weekRes.status === "fulfilled") {
                    const rows = weekRes.value.data.data ?? []
                    setMonthlyTrend(groupByMonth(rows))
                }
            })
            .finally(() => {
    ...
    const attendanceRate = totalAttendance > 0
        ? normalizePercent(Number(((totalPresent / totalAttendance) * 100).toFixed(1)))
        : 0

    const warningCount = warnings.filter(w => !Boolean(w.processed)).length

    const warningClasses = classData
        .filter(r => Number(r.tong_luot_diem_danh) > 0)
        .map(r => {
            const total = Number(r.tong_luot_diem_danh) || 0
            const absent = Number(r.vang) || 0

            return {
                id: r.lop_mon_hoc_id,
                ma_lop: r.ma_lop,
                ten_hoc_phan: r.ten_hoc_phan,
                absenceRate: total > 0 ? normalizePercent(Number(((absent / total) * 100).toFixed(1))) : 0,
                absentCount: absent,
            }
        })
        .sort((a, b) => b.absenceRate - a.absenceRate)
        ...
        <div className="flex flex-col w-full pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                ...
                <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm flex flex-col justify-between">
                    ...
                    <h3 className="text-3xl font-medium text-[#185FA5]">
                        {attendanceRate.toFixed(1)}%
                    </h3>

                    <div className="mt-2 w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                        <div
                            className="bg-[#185FA5] h-full rounded-full"
                ...
                <div className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm flex flex-col justify-between relative overflow-hidden">
                    ...
                    <h3 className="text-3xl font-medium text-red-600 relative z-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                <div className="lg:col-span-2 bg-white border border-slate-200 rounded-lg shadow-sm p-6 flex flex-col">
                    ...
                    {monthlyTrend.length === 0 ? (
                        <div className="flex-1 flex items-center justify-center text-slate-400 text-sm min-h-[240px]">
                            Chưa có dữ liệu điểm danh
                        </div>
                    ) : (
                        <div className="flex-1 flex flex-col justify-end min-h-[240px]">
                            <div className="relative h-48 w-full flex items-end justify-around pb-6 border-b border-slate-100 overflow-hidden">
                                <div className="absolute inset-0 flex flex-col justify-between pb-6 pointer-events-none">
                                    {[100, 75, 50, 25].map(v => (
                                        <div key={v} className="flex items-center gap-2">
                                            <span className="text-[10px] text-slate-400 w-7 text-right">{v}%</span>
                                ...
                                {monthlyTrend.map(m => {
                                    const safeRate = normalizePercent(m.rate)
                                    const barHeight = Math.min((safeRate / chartMax) * 170, 170)
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div
                                                className="w-8 md:w-10 lg:w-12 rounded-t-sm transition-all bg-[#185FA5]"
                                                style={{
                                                    height: `${barHeight}px`,
                                                }}
                                                title={`${monthLabel(m.month)}: ${safeRate.toFixed(1)}%`}
                                            />
                                        </div>
                                    )
                                })}
                ...
                <div className="lg:col-span-1 bg-white border border-slate-200 rounded-lg shadow-sm p-6 flex flex-col">
                    <div className="flex justify-between items-center mb-5">
                        <h3 className="font-medium text-slate-800 text-lg">Cảnh báo tỷ lệ vắng</h3>
                        <span className="bg-red-50 text-red-600 px-2 py-1 rounded text-[10px] font-medium uppercase tracking-wider">
                            Top 5
                        </span>
                    </div>
                        ...
                        <div className="flex flex-col gap-4 flex-1">
                            {warningClasses.map(item => (
                                <div key={item.id} className="flex justify-between items-start group">
                                    ...
                                    <div className="text-right shrink-0">
                                        <span className="font-medium text-red-600">{item.absenceRate.toFixed(1)}%</span>
                                        <p className="text-[10px] font-medium text-slate-400 uppercase">Vắng</p>
                                    </div>
                                </div>
                            ))}
            ...
            <div className="bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col overflow-hidden">
                    ...
                    <table className="w-full text-left text-sm text-slate-600">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {classData.length === 0 ? (
                                ...
                                [...classData]
                                    .sort((a, b) => {
                                        const ra = Number(a.tong_luot_diem_danh) > 0
                                            ? Number(a.vang) / Number(a.tong_luot_diem_danh)
                                            : 0

                                        const rb = Number(b.tong_luot_diem_danh) > 0
                                            ? Number(b.vang) / Number(b.tong_luot_diem_danh)
                                            : 0

                                        return rb - ra
                                    })
                                    .map(row => {
                                        const total = Number(row.tong_luot_diem_danh) || 0
                                        const absent = Number(row.vang) || 0

                                        const absenceRate = total > 0
                                            ? normalizePercent(Number(((absent / total) * 100).toFixed(1)))
                                            : 0
                                        ...
                                        return (
                                            <tr key={row.lop_mon_hoc_id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-5 py-4 font-medium text-slate-800 max-w-[200px]">
                                                    <div className="truncate" title={row.ten_hoc_phan}>
                                                        {row.ten_hoc_phan}
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-5 py-4 text-right font-medium text-[#15803d]">
                                                    {(Number(row.co_mat) || 0).toLocaleString("vi-VN")}
                                                </td>

                                                <td className="px-5 py-4 text-right font-medium text-red-500">
                                                    {absent.toLocaleString("vi-VN")}
                                                </td>
                ...
                <div className="p-4 border-t border-slate-100 flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
                    <span className="text-sm font-medium text-slate-500">
                        Hiển thị {classData.length} lớp
                    </span>
