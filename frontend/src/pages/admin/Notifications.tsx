...
function CreateNotificationModal({
  ...
  useEffect(() => {
    if (targetType === "course_class" && courseClasses.length === 0) {
      setLoadingClasses(true)
      api
        .get("/course-classes", { params: { limit: 200 } })
        .then((res) => setCourseClasses(res.data.data?.data ?? res.data.data ?? []))
        .catch(() => {})
        .finally(() => setLoadingClasses(false))
    }
  }, [targetType, courseClasses.length])

  const filteredClasses = courseClasses.filter(
    (c) =>
      c.ma_lop.toLowerCase().includes(classSearch.toLowerCase()) ||
      c.ten_hoc_phan.toLowerCase().includes(classSearch.toLowerCase()) ||
      c.ma_hoc_phan?.toLowerCase().includes(classSearch.toLowerCase())
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (targetType === "course_class" && !selectedClassId) {
      setError("Vui lòng chọn lớp học phần.")
      return
    }

    if (targetType === "student_ids" && !studentIdsInput.trim()) {
  ...
  const TARGET_OPTIONS: {
  ...
  }[] = [
    {
      ...
      desc: "Gửi đến toàn bộ tài khoản trong hệ thống",
      icon: <Globe size={16} />,
    },
    {
      type: "role",
      label: "Theo vai trò",
      desc: "Chọn nhóm: sinh viên, giảng viên, admin",
      icon: <Users size={16} />,
    },
    {
      type: "course_class",
      label: "Theo lớp học phần",
      desc: "Gửi đến tất cả sinh viên trong một lớp cụ thể",
      icon: <BookOpen size={16} />,
    },
    {
      type: "student_ids",
      label: "Sinh viên cụ thể",
      desc: "Gửi đến từng sinh viên theo ID",
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50 shrink-0">
          ...
          {!success && (
            <button
              onClick={onClose}
              disabled={sending}
              className="p-1.5 hover:bg-slate-200 rounded-full text-slate-400 transition-colors disabled:opacity-50"
            >
              <X size={18} />
            </button>
          )}
              ...
              {targetType === "student_ids" && (
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Nhập ID tài khoản sinh viên hoặc ID sinh viên theo cấu hình backend.
                  </p>
                </div>
              )}
            </form>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <span className="font-medium text-[15px] mb-0.5 block text-slate-700">
                          {notif.title}
                        </span>

                        {notif.class_name && (
                          <span className="text-xs text-[#185FA5] block mb-0.5">
                            {notif.class_name}
                          </span>
                        )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
              <div className="flex items-center gap-2">
                ...
                {selected.class_name && (
                  <span className="px-2.5 py-1 rounded-full text-xs font-medium border bg-blue-50 text-[#185FA5] border-blue-100">
                    {selected.class_name}
                  </span>
                )}
              </div>
            ...
            <div className="px-6 py-5 flex flex-col gap-4">
              <h2 className="text-lg font-medium text-[#185FA5] leading-snug">
                {selected.title}
              </h2>
