  ...
  return (
    <div className="flex flex-col gap-6 w-full pb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button className="flex items-center justify-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-5 py-2.5 rounded-md font-medium text-sm transition-colors shadow-sm">
          <Download size={16} />
          Xuất báo cáo PDF
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#185FA5] rounded-xl p-6 text-white shadow-sm relative overflow-hidden">
          <div className="absolute right-0 top-0 opacity-10 transform translate-x-4 -translate-y-4">
            <BarChart3 size={100} />
          </div>
          <div className="relative z-10">
            <p className="text-blue-200 font-medium text-xs mb-2 uppercase tracking-wider">Tỷ lệ đi học trung bình</p>
            <h2 className="text-4xl font-medium mb-4">{mockReports.tong_quan.ti_le_di_hoc}%</h2>
            <div className="flex items-center gap-2 text-sm font-medium bg-white/15 w-fit px-3 py-1.5 rounded-md">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-slate-500 text-sm">Sinh viên rủi ro cao</p>
              <h3 className="text-2xl font-medium text-slate-800">{mockReports.tong_quan.sv_canh_bao}</h3>
            </div>
          </div>
          <p className="text-sm text-slate-400">Cần gửi cảnh báo (Vắng &gt; 20%)</p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col justify-center">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-slate-500 text-sm">Tổng số buổi đã dạy</p>
              <h3 className="text-2xl font-medium text-slate-800">{mockReports.tong_quan.so_buoi_da_day}</h3>
            </div>
          </div>
          <p className="text-sm text-slate-400">Trên {mockReports.tong_quan.tong_so_lop} lớp học phần</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h3 className="font-medium text-slate-800 text-lg">Biểu đồ chuyên cần theo tuần</h3>
          <select className="border border-slate-200 rounded-md px-3 py-1.5 text-sm text-slate-700 focus:outline-none focus:border-[#185FA5]">
        ...
        <div className="p-8 h-80 flex items-end justify-around gap-4 md:gap-8 border-b border-slate-100 relative">
          ...
          {mockReports.bieu_do_chuyen_can.map((item, index) => (
            <div key={index} className="relative flex flex-col items-center flex-1 h-full justify-end z-10 group">
              <div
                className="w-full max-w-16 bg-[#185FA5]/15 rounded-t-md transition-all duration-300 group-hover:bg-[#185FA5]/25 relative"
                style={{ height: `${(item.value / 100) * 100}%` }}
              >
                <div
                  className="absolute bottom-0 w-full bg-[#185FA5] rounded-t-md transition-all duration-300"
                  style={{ height: `${(item.value / chartMax) * 100}%` }}
                ></div>
                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs font-medium px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                  {item.value}%
                </div>
              </div>
              <span className="mt-4 text-xs font-medium text-slate-500 uppercase">{item.label}</span>
            </div>
          ))}
        </div>
      </div>
