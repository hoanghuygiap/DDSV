...
import { ArrowLeft, RefreshCcw, Wifi, MapPin, StopCircle, UserCheck, Play, Loader2, Bug } from "lucide-react"
...
export default function LiveAttendanceQR() {
  ...
  useEffect(() => {
    ...
    // Call Immediately to get the very first list and fresh QR (nếu chưa có)
    if (!qrToken) {
      handleRefreshQR();
    }
    fetchRealtime();
    ...
    const interval = setInterval(() => {
      ...
      // Polling Realtime data every 2 seconds
      if (localTimer % 2 === 0) {
        fetchRealtime();
      }
    }, 1000)
  ...
  const qrPayload = JSON.stringify({ token: qrToken, session_id: Number(sessionId) })
    ...
    <div className="flex flex-col gap-6 w-full pb-8 h-[calc(100vh-80px)]">
      ...
      <div className="flex items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-4">
          <Link to={`/dashboard/my-classes/${sessionInfo.lop_mon_hoc_id}`} className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-[#00a8cc] hover:bg-blue-50 transition-colors">
            <ArrowLeft size={20} />
          </Link>
          <div>
        ...
        {isLive ? (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 px-5 py-2.5 rounded-lg font-bold text-sm transition-colors border border-red-200 disabled:opacity-50"
            >
          ...
          sessionInfo.trang_thai !== "da_ket_thuc" && sessionInfo.trang_thai !== "huy" && (
            <button
              ...
              className="flex items-center gap-2 bg-[#00a8cc] hover:bg-[#008ba8] text-white px-5 py-2.5 rounded-lg font-bold text-sm transition-colors disabled:opacity-50 shadow-md shadow-[#00a8cc]/20"
            >
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col items-center justify-center p-8 relative overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-md flex flex-col items-center">
              ...
              <div className="w-full h-full border-4 border-[#1a3a5f] rounded-lg p-2 flex items-center justify-center bg-white relative overflow-hidden">
                {!isLive ? (
                  <div className="flex flex-col items-center text-slate-400 gap-3 text-center p-4">
                    ...
                    <p className="font-bold">Màn hình đang khóa</p>
                  ...
                  <div className="w-full h-full bg-white flex items-center justify-center relative overflow-hidden">
                    <QRCodeSVG value={qrPayload} size={260} level="H" includeMargin={true} fgColor="#1a3a5f" />
            ...
            <h2 className="text-2xl font-black text-[#1a3a5f] mb-2 text-center">
              ...
              <div className="flex gap-4 w-full">
                {sessionInfo.require_wifi ? (
                  <div className="flex-1 bg-blue-50 border border-blue-100 rounded-lg p-3 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-500 font-bold uppercase">Mạng Bắt buộc</p>
                      <p className="text-sm font-bold text-blue-700">Trường DHTL</p>
                    </div>
                  </div>
                ) : null}
                {sessionInfo.require_gps ? (
                  <div className="flex-1 bg-indigo-50 border border-indigo-100 rounded-lg p-3 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-500 font-bold uppercase">Khoảng cách</p>
                      <p className="text-sm font-bold text-indigo-700">{sessionInfo.ban_kinh_gps_m} mét</p>
                    </div>
                  </div>
                ) : null}
              </div>
        ...
        <div className="lg:col-span-1 flex flex-col gap-6 h-full min-h-0">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 shrink-0 relative overflow-hidden">
            ...
            <h3 className="font-bold text-[#1a3a5f] text-lg mb-4 flex justify-between items-center">
              Tiến độ kiểm diện
            ...
            <div className="flex items-end justify-between mb-2">
              <span className="text-4xl font-black text-[#00a8cc]">{studentsHistory.length}</span>
              <span className="text-slate-500 font-medium mb-1">Đã Check-in</span>
            </div>

            <div className="flex items-center gap-2 text-sm text-[#00a8cc] font-bold bg-blue-50 p-3 rounded-lg border border-blue-100 mt-4">
              <UserCheck size={18} />
            ...
            <div className="p-4 overflow-y-auto flex-1 flex flex-col gap-3">
              {studentsHistory.map((sv, i) => (
                <div key={i} className={`flex items-center justify-between p-3 border rounded-lg animate-in fade-in slide-in-from-right-4 duration-300 ${sv.trang_thai === 'tre' ? 'border-amber-200 bg-amber-50' : 'border-emerald-100 bg-emerald-50/50'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 ${sv.trang_thai === 'tre' ? 'bg-amber-200 text-amber-700' : 'bg-emerald-100 text-emerald-600'}`}>
                      {sv.trang_thai === 'tre' ? 'TRỄ' : <UserCheck size={14} />}
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-slate-800">{sv.student_name}</h4>
