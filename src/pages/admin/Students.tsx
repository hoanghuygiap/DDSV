...
export default function StudentsPage() {
  ...
  useEffect(() => { fetchStudents(page, keyword) }, [page, keyword, filterKhoa, filterNganh, filterLop, filterStatus])

  // ─── Derived ─────────────────────────────────────────────
  const filteredMajors = majors.filter(m => !filterKhoa || m.khoa_id === filterKhoa)

  const filteredClasses = classes.filter(c => {
    if (filterNganh) return c.nganh_id === filterNganh
    if (filterKhoa) {
      const majorIds = majors.filter(m => m.khoa_id === filterKhoa).map(m => m.id)
      return majorIds.includes(c.nganh_id)
    }
    return true
  })

  const activeFilterCount =
    (filterKhoa ? 1 : 0) + (filterNganh ? 1 : 0) + (filterLop ? 1 : 0) + (filterStatus !== "" ? 1 : 0)
  ...
  return (
    <div className="flex flex-col w-full pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-4">
        <div>
          <h1 className="text-[22px] font-medium text-[#185FA5]">Quản lý Sinh viên</h1>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="flex gap-2 w-full md:w-80">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="block w-full pl-9 pr-8 py-2 border border-slate-200 rounded-md text-sm bg-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-[#185FA5] focus:border-[#185FA5] shadow-sm"
                placeholder="Tên, mã SV, email, SĐT... (Enter)"
              />
              {keywordInput && (
                <button
                  onClick={clearSearch}
          ...
          <button onClick={openCreate}
            className="flex items-center gap-2 bg-[#185FA5] hover:bg-[#1254a0] text-white rounded-md px-4 py-2 text-sm font-medium shadow-sm transition-colors whitespace-nowrap h-[38px]">
            <UserPlus size={18} />
      ...
      {/* FILTER BAR */}
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm px-4 py-3 mb-4 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 text-slate-500 shrink-0">
          <SlidersHorizontal size={15} />
          <span className="text-xs font-medium uppercase tracking-wider">Bộ lọc</span>
          {activeFilterCount > 0 && (
            <span className="w-5 h-5 rounded-full bg-[#185FA5] text-white text-[10px] font-medium flex items-center justify-center">
              {activeFilterCount}
            </span>
          )}
        </div>
        ...
        <select
          ...
          className="border border-slate-200 rounded-md px-3 py-1.5 text-sm bg-white text-slate-600 focus:outline-none focus:ring-1 focus:ring-[#185FA5] min-w-[160px]"
        >
        ...
        <select
          value={filterNganh}
          onChange={(e) => {
            setFilterNganh(e.target.value ? Number(e.target.value) : "")
            setFilterLop("")
            setFilterStatus("")
            setPage(1)
          }}
          disabled={filteredMajors.length === 0}
          className="border border-slate-200 rounded-md px-3 py-1.5 text-sm bg-white text-slate-600 focus:outline-none focus:ring-1 focus:ring-[#185FA5] min-w-[160px] disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <select
          value={filterLop}
          onChange={(e) => {
            setFilterLop(e.target.value ? Number(e.target.value) : "")
            setFilterStatus("")
            setPage(1)
          }}
        ...
        {/* Trạng thái — chỉ hiện khi đã chọn lớp */}
        {filterLop !== "" && (
          <select
            value={filterStatus}
            onChange={(e) => { setFilterStatus(e.target.value as "" | "true" | "false"); setPage(1) }}
            className="border border-slate-200 rounded-md px-3 py-1.5 text-sm bg-white text-slate-600 focus:outline-none focus:ring-1 focus:ring-[#185FA5] min-w-[150px]"
          >
        ...
        {activeFilterCount > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-red-600 hover:text-red-700 border border-red-100 bg-red-50 hover:bg-red-100 rounded-md transition-colors"
          >
            <X size={12} />
            Xóa bộ lọc
          </button>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden mb-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="text-xs uppercase text-slate-500 bg-slate-50/80 border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 font-medium tracking-wider">Sinh viên</th>
                    ...
                    <tr key={sv.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-[#185FA5]/10 flex items-center justify-center font-medium text-xs text-[#185FA5] shrink-0">
                            {initials}
                          </div>
                          <div>
                            <button
                              onClick={() => navigate(`/dashboard/students/${sv.id}`)}
                              className="font-medium text-[#185FA5] text-[15px] hover:underline text-left">
                              {sv.ho_ten || "—"}
                            </button>
                      ...
                      <td className="px-6 py-4">
                        <p className="text-[#185FA5] font-medium text-sm">{sv.email || "—"}</p>
                      ...
                      <td className="px-6 py-4 text-center">
                        {sv.kich_hoat ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 text-red-600 rounded-full text-xs font-medium border border-red-200">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-3">
                          ...
                          <button onClick={() => openEdit(sv)}
                            className="text-slate-400 hover:text-[#185FA5] transition-colors" title="Chỉnh sửa">
                            <Pencil size={17} />
                          </button>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-3">
          <span className="text-sm text-slate-500">
            ...
            trong <span className="font-medium text-slate-700">{total}</span> sinh viên
          </span>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              {!editTarget && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-medium text-slate-600 mb-1 block">Tên đăng nhập <span className="text-red-500">*</span></label>
                    <input className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#185FA5] focus:border-[#185FA5]"
                      value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })}
                      placeholder="a12345" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-slate-600 mb-1 block">Mật khẩu <span className="text-red-500">*</span></label>
                    <input type="password" className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#185FA5] focus:border-[#185FA5]"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-medium text-slate-600 mb-1 block">Họ và tên <span className="text-red-500">*</span></label>
                  <input className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#185FA5] focus:border-[#185FA5]"
                ...
                {!editTarget && (
                  <div>
                    <label className="text-xs font-medium text-slate-600 mb-1 block">Mã sinh viên <span className="text-red-500">*</span></label>
                    <input className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#185FA5] focus:border-[#185FA5]"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-medium text-slate-600 mb-1 block">Email <span className="text-red-500">*</span></label>
                  <input type="email" className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#185FA5] focus:border-[#185FA5]"
                    value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                    placeholder="sv@thanglong.edu.vn" />
                </div>
                {!editTarget && (
                  <div>
                    <label className="text-xs font-medium text-slate-600 mb-1 block">Số điện thoại <span className="text-red-500">*</span></label>
                    <input className="w-full border border-slate-200 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#185FA5] focus:border-[#185FA5]"
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50/50 rounded-b-lg">
              ...
              <button onClick={handleSave} disabled={isSaving}
                className="flex items-center gap-2 bg-[#185FA5] hover:bg-[#1254a0] text-white rounded-md px-5 py-2 text-sm font-medium shadow-sm disabled:opacity-60">
          ...
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-sm">
            ...
            <div className="px-6 py-5 space-y-4">
              <p className="text-sm text-slate-500">
                ...
                <span className="font-medium text-slate-800">{lockTarget.ho_ten || lockTarget.username}</span>.
              </p>
              <div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50/50 rounded-b-lg">
              ...
              <button onClick={handleLock} disabled={isLocking}
                className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white rounded-md px-5 py-2 text-sm font-medium disabled:opacity-60">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-slate-500 mb-6">
              ...
              <span className="font-medium text-slate-800">{unlockTarget.ho_ten || unlockTarget.username}</span>?
            </p>
            <div className="flex items-center justify-end gap-3">
              <button onClick={() => setUnlockTarget(null)} disabled={isLocking}
                className="px-4 py-2 text-sm font-medium text-slate-600">Hủy</button>
              <button onClick={handleUnlock} disabled={isLocking}
                className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md px-5 py-2 text-sm font-medium disabled:opacity-60">
                {isLocking && <Loader2 size={14} className="animate-spin" />}
                Mở khóa
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-slate-500 mb-6">
              ...
              <span className="font-medium text-slate-800">{deleteTarget.ho_ten || deleteTarget.username}</span>?
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button onClick={handleDelete} disabled={isDeleting}
                className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white rounded-md px-5 py-2 text-sm font-medium disabled:opacity-60">
